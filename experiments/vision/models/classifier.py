...
class ResNetClassifier(nn.Module):
    ...
    def __init__(self, 
        ...
        self.enc_manifold = getattr(self.encoder, "output_manifold", self.encoder.manifold)
    ...
    def check_manifold(self, x):
        enc_is_euclidean = self.enc_manifold is None

        if enc_is_euclidean:
            if self.dec_type == "euclidean":
                return x
            ...
            if self.dec_type == "euclidean":
                ...
                if self.dec_manifold is None:
                    return x
                same_manifold = (
                    self.dec_manifold is self.enc_manifold
                    or torch.allclose(self.enc_manifold.k, self.dec_manifold.k)
                )
                if not same_manifold:
