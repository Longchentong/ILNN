...
def get_hyperbolic_convolution_block(manifold, 
    ...
    return nn.Sequential(
            ...
            LorentzDropout(manifold=manifold, p = share.share_dropout_rate) if share.share_dropout_rate > 0 else nn.Identity(),
...
class HyperbolicCNN(nn.Module):
    def __init__(self, 
        ...
        self.dropout = LorentzDropout(manifold=self.manifolds[-1], p = share.share_dropout_rate) if share.share_dropout_rate > 0 else nn.Identity()
    ...
    def forward(self, x):
        ...
        # print("---------------------x mainfolds is finate? ->", torch.isfinite(x).all(), "max|x| =", x.abs().amax())
        ...
        for i in range(self.num_layers):
            ...
            res = self.shortcut(x)
        # print("---------------------x conv is finate? ->", torch.isfinite(x).all(), "max|x| =", x.abs().amax())
        
        x = x.unsqueeze(2) #[b, l, c] -> [b, l, 1, c]
        x = self.manifolds[-1].lorentz_flatten(x)
        # print("---------------------x lorentz_flatten is finate? ->", torch.isfinite(x).all(), "max|x| =", x.abs().amax())

        x = self.fc_layer(x)
        # print("-------------------x fc is finate? ->", torch.isfinite(x).all(), "max|x| =", x.abs().amax())

        x = self.activations[-1](x)
        x = self.dropout(x)
        x = self.mlr(x)
        # print("-------------------x mlr is finate? ->", torch.isfinite(x).all(), "max|x| =", x.abs().amax())
        if not torch.isfinite(x).all():
            raise RuntimeError("x is not finate")
